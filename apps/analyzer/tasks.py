...
def _maybe_send_welcome_email(run) -> None:
    """Send a welcome email when this is the user's very first completed analysis."""
    try:
        completed_count = AnalysisRun.objects.filter(
            email=run.email,
        ...
        analysis_url = f"{frontend_url}/dashboard/{run.slug}"
        from apps.accounts.email_utils import send_welcome_email
        send_welcome_email(run.email, run.brand_name or run.url, analysis_url)
    except Exception:
        logger.exception("Failed to send welcome email for run %d", run.id)
