...
class DownloadInvoiceView(APIView):
    ...
    def get(self, request):
        ...
        # Use caller-supplied payment_id if it belongs to this email, else fall back to latest
        payment_id_param = request.query_params.get("payment_id", "").strip()
        if payment_id_param and InvoiceRecord.objects.filter(email=email, payment_id=payment_id_param).exists():
            resolved_payment_id = payment_id_param
        else:
            resolved_payment_id = sub.last_invoice_payment_id

        pdf, err = fetch_payment_invoice_pdf(resolved_payment_id)
        if not pdf:
        ...
        safe_name = resolved_payment_id.replace("/", "_")[:80]
...
class PlanPricesView(APIView):
    """GET /api/payments/plan-prices/ — return plan prices in the DodoPlanPrice shape."""
    permission_classes = [AllowAny]

    def get(self, request):
        def _make_price(plan_key):
            cfg = PLAN_LIMITS.get(plan_key)
            if not cfg:
                return None
            price_gbp = cfg["price_gbp"]
            return {
                "currency": "gbp",
                "amount": price_gbp,
                "amount_minor": round(price_gbp * 100),
                "interval": "month",
                "interval_count": 1,
                "prices_by_currency": {"gbp": price_gbp},
            }

        return Response({
            "starter": _make_price("starter"),
            "pro": _make_price("pro"),
            "business": _make_price("business"),
            "source": "fallback",
        })


class InvoiceListView(APIView):
    """GET /api/payments/invoices/?email= — full invoice history from InvoiceRecord."""
    permission_classes = [AllowAny]
    ...
    def get(self, request):
        ...
        records = InvoiceRecord.objects.filter(email=email).order_by("-created_at")
        items = [
            {
